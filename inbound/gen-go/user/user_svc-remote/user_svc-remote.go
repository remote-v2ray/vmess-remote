...
func Usage() {
  ...
  fmt.Fprintln(os.Stderr, "  void Add(MemoryUser u)")
  fmt.Fprintln(os.Stderr, "  void Close()")
  fmt.Fprintln(os.Stderr, "  GetUserResponse Get(string userHash)")
  fmt.Fprintln(os.Stderr, "  void Remove(string email)")
  fmt.Fprintln(os.Stderr)
  os.Exit(0)
}
...
func main() {
  ...
  client := user.NewUserSvcClient(thrift.NewTStandardClient(iprot, oprot))
  ...
  case "Close":
    if flag.NArg() - 1 != 0 {
      fmt.Fprintln(os.Stderr, "Close requires 0 args")
      flag.Usage()
    }
    fmt.Print(client.Close(context.Background()))
    fmt.Print("\n")
    break
  case "Get":
    if flag.NArg() - 1 != 1 {
